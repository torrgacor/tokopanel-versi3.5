...
export default async function InvoicePage({ params }: { params: { id: string } }) {
  ...
  // Get egg details if selectedEggId exists
  let selectedEggName = null
  let selectedEggPrice = 0
  
  if (payment.selectedEggId) {
    const config = pterodactylConfig[payment.serverType as keyof typeof pterodactylConfig]
    const egg = config?.eggs?.find((e: any) => e.id === payment.selectedEggId)
    if (egg) {
      selectedEggName = egg.nama
      selectedEggPrice = egg.harga
    } else {
      selectedEggName = `Egg ID: ${payment.selectedEggId}`
    }
  }
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            <InvoiceDetails
              ...
              selectedEggName={selectedEggName}
              selectedEggPrice={selectedEggPrice}
